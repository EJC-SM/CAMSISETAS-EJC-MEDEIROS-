import type { Cor, ItemPedido, Produto } from '../state/types';
import { digitsOnly, sanitizeText } from './security';

export interface ValidationResult {
  valid: boolean;
  message?: string;
}

export function validateNome(value: string): ValidationResult {
  const nome = sanitizeText(value);
  if (!nome) return { valid: false, message: 'Digite seu nome completo.' };
  if (nome.length < 3) return { valid: false, message: 'Nome muito curto.' };
  if (nome.length > 80) return { valid: false, message: 'Nome muito longo.' };
  if (!/^[\p{L}0-9 .\-']+$/u.test(nome)) {
    return { valid: false, message: 'Nome possui caracteres inválidos.' };
  }
  return { valid: true };
}

export function validateEquipe(value: string, allowed: string[]): ValidationResult {
  if (!value) return { valid: false, message: 'Selecione sua equipe.' };
  if (allowed.length > 0 && !allowed.includes(value)) {
    return { valid: false, message: 'Equipe inválida.' };
  }
  return { valid: true };
}

export function validateTelefone(value: string): ValidationResult {
  if (!value.trim()) return { valid: false, message: 'Informe seu WhatsApp.' };
  const d = digitsOnly(value);
  if (!(d.length === 10 || d.length === 11)) {
    return { valid: false, message: 'WhatsApp deve ter DDD + número (10 ou 11 dígitos).' };
  }
  return { valid: true };
}

export function validateQuantidade(value: number): ValidationResult {
  if (!Number.isFinite(value) || !Number.isInteger(value)) {
    return { valid: false, message: 'Quantidade inválida.' };
  }
  if (value < 1) return { valid: false, message: 'Quantidade mínima é 1.' };
  if (value > 99) return { valid: false, message: 'Quantidade máxima é 99.' };
  return { valid: true };
}

export function validateItemPedido(
  item: Partial<ItemPedido>,
  produtos: Produto[],
  cores: Cor[],
): ValidationResult {
  const produto = produtos.find((p) => p.tipo === item.produto || p.id === item.produto);
  if (!produto) return { valid: false, message: 'Escolha um modelo.' };
  if (!item.tamanho || !produto.tamanhos.includes(item.tamanho)) {
    return { valid: false, message: `Escolha o tamanho de ${produto.tipo}.` };
  }
  const golasValidas = produto.golas.filter((g) => g && g !== '—');
  if (golasValidas.length > 0 && (!item.gola || !golasValidas.includes(item.gola))) {
    return { valid: false, message: `Escolha a gola de ${produto.tipo}.` };
  }
  if (!item.cor) return { valid: false, message: `Escolha a cor de ${produto.tipo}.` };
  const corValida = cores.some((c) => c.nome === item.cor);
  if (cores.length > 0 && !corValida) return { valid: false, message: 'Cor inválida.' };
  if ((produto.cores_excluidas || []).includes(item.cor)) {
    return { valid: false, message: `${item.cor} não está disponível para ${produto.tipo}.` };
  }
  return validateQuantidade(Number(item.quantidade));
}

export function getPreco(produto: Produto, tamanho: string): number {
  const p = produto.precos || {};
  if (p.all != null) return Number(p.all) || 0;
  if (tamanho === 'EGG') return Number(p.EGG ?? p.EG ?? p['P-GG']) || 0;
  if (tamanho === 'XG') return Number(p.XG ?? p['P-GG']) || 0;
  if (tamanho === 'EG') return Number(p.EG ?? p['P-GG']) || 0;
  return Number(p['P-GG']) || 0;
}
